...
import { loadRecords, saveRecords, deleteRecords } from './store'
...
export function buildReminderUI({ title, identifier, dueDate, notes }: ReminderAttributes) {
  ...
  return {
    ...
    compactTrailing: d.getTime() - now.getTime() < config.widgetTimerInterval * 60000 ? (
      <TimerIntervalLabel
        from={Date.now()}
        to={d.getTime()}
        font={15}
        ...
        // countsDown={false}
        showsHours={false}
      />) : (<Text foregroundStyle="white" font={15}>{timeText}</Text>),

    minimal: d.getTime() - now.getTime() < config.widgetTimerInterval * 60000 ? (
      <TimerIntervalLabel
        from={Date.now()}
        to={d.getTime()}
        font={15}
        ...
        // countsDown={false}
        showsHours={false}
      />) : (<Text foregroundStyle="white" font={10.5}>{timeText}</Text>)
  }
}
